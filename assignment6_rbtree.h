...
private:
	...
	int diameter(Node<K, V> *node) const {
		...
		int h_right = height(node->right); //height of right subtree
		int h_left = height(node->left); //height of left subtree
		int d = h_right + h_left;
		return 2 + d;
